    ...
    private static void cargarPersonas(TreeMap<String, Persona> mapa, String archivo) {
        try (Reader reader = new FileReader(archivo)) {
            ...
            for (Map<String, String> entrada : datos) {
                ...
                Persona p = PersonaFactory.crear(tipo, nombre, new ZonaComun("JAJAJ")); // todo falta agregar la zona donde se encuentra la persona.
                mapa.put(p.getId(), p); // todo esto lo hace el add del controlador. Nos estamos metiendo con logica del controlador. Esta bien?
            }
        } catch (IOException e) {
